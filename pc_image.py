...
def enhance_topo_map(topo_map, scan_id, scan_count):
...
def flatten_normal_map(normal_map):
	...
	normal_map_smoothed_nans = skimage.filters.gaussian((~normal_map_mask).astype(float), sigma=sigma, multichannel=True)
...
def create_topo_map(depth_map, scan_id, scan_count, sigma):
	...
	depth_map_smoothed_nans = skimage.filters.gaussian((~depth_map_mask).astype(float), sigma=sigma)
...
def rgb_save_images(save_path, scan_id, depth_map, normal_map, topo_maps, enhanced_topo_maps, img_type, transparency):
	...
	with warnings.catch_warnings():
		...
		if img_type != 'tif':
			...
			normal_map_tif.save(save_path+scan_id+'_normal_map.'+img_type, dpi=(1800,1800), quality=100)
	...
	with warnings.catch_warnings():
		...
		for key, topo_map in topo_maps.items():
			...
			if img_type != 'tif':
				...
				topo_map_tif.save(save_path+'topo_maps/'+scan_id+'_topo_map_' + key + '.'+img_type, dpi=(1800,1800), quality=100)
	...
	with warnings.catch_warnings():
		...
		for key, enhanced_topo_map in enhanced_topo_maps.items():
			...
			if img_type != 'tif':
				imageio.imwrite(save_path + 'enhanced_topo_maps/'+scan_id+'_enhanced_topo_map_' + key + '.'+img_type, enhanced_topo_map_img, transparency=rgb_transparency, quality=10, pixelformat='yuvj444p')
			else:
	...
	with warnings.catch_warnings():
		...
		for key, enhanced_topo_map in enhanced_topo_maps.items():
			...
			if img_type != 'tif':
				imageio.imwrite(save_path + 'blended_maps/'+scan_id+'_blended_map_' + key + '.'+img_type, blended_map_img, transparency=rgb_transparency, quality=10, pixelformat='yuvj444p')
			else:
	...
	with warnings.catch_warnings():
		...
		for key, enhanced_topo_map in enhanced_topo_maps.items():
			...
			if img_type != 'tif':
				...
				enhanced_topo_tif = Image.fromarray(enhanced_topo_map_img)
				enhanced_topo_tif.save(save_path + 'enhanced_topo_maps/'+scan_id+'_enhanced_topo_map_' + key + '_grey.'+img_type, dpi=(1800,1800),quality=100)
	...
	with warnings.catch_warnings():
		...
		for key, enhanced_topo_map in enhanced_topo_maps.items():
			...
			if img_type != 'tif':
				...
				blended_map_tif = Image.fromarray(blended_map_img)
				blended_map_tif.save(save_path + 'blended_maps/'+scan_id+'_blended_map_' + key + '_grey.'+img_type, dpi=(1800,1800), quality=100)